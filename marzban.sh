...
backup_command() {
    ...
    if [ -n "$db_type" ]; then
        ...
        case $db_type in
            mariadb)
                if ! docker exec "$container_name" mariadb-dump -u root -p"$MYSQL_ROOT_PASSWORD" --all-databases --events  --ignore-database=mysql --ignore-database=information_schema --ignore-database=performance_schema --ignore-database=sys --triggers > "$temp_dir/db_backup.sql" 2>>"$log_file"; then
