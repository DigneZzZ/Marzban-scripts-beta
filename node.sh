...
install_marzban_node() {
    ...
    # Function to prompt user for port and validate input
    prompt_for_port() {
        local port_name=$1
        ...
        while true; do
            read -p "Enter port for $port_name (default: $default_port): " port
            port=${port:-$default_port}
            if [[ $port -ge 1 && $port -le 65535 ]]; then
                echo $port
                return
            else
                colorized_echo red "Invalid port. Please enter a port number between 1 and 65535."
            fi
        done
    }

    SERVICE_PORT=$(prompt_for_port "SERVICE_PORT" $DEFAULT_SERVICE_PORT)
    XRAY_API_PORT=$(prompt_for_port "XRAY_API_PORT" $DEFAULT_XRAY_API_PORT)
...
usage() {
    colorized_echo red "Usage: $APP_NAME [command] [install directory]"
    echo
    echo "Commands:"
